...
Csv::Csv(const std::string& file)
{
	...
	while (ifs) {
		Line line;
		parse_line(ifs, line);
		if (line) {	// comment because some field can be empty (e.g preview_link sometimes doesn't exist)
...
std::istream& Csv::parse_line(std::istream& is, Line& line)
{
	for (char ch = is.peek(); is; ch=is.peek()) {
		...
		// used to not add empty field in case when file almost readed or if char is just space
		else if (std::isspace(ch) || ch == EOF) {
			is.get();	
			continue;
		}
		else {
