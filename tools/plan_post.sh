...
# convert it and post at specified time hh:mm:ss
...
[ ! "$time" ] && die "specify time in which post will occur: $time"
...
# convert in unix epoch format
post_at=$(date +%s -d "$time")
exit_if_failed
now=$(date +%s)
exit_if_failed

if [ $post_at -lt $now ]; then
        die "time for post less then current time\n"
fi

wait_for=$((post_at-now))
sleep $wait_for && ../make_post/start.sh "$html_post"
